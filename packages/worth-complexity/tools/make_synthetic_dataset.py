...
log in the shape Epic Clarity produces, and the ANSI X12 835 remittances that
pay for it. It exists so the Layer A scorer, the linkage, the reference curve
and the adequacy ratio can all be exercised end to end before any real data
arrives.
...
def write_clinical(out: Path, encs: list[Encounter], rng: random.Random) -> None:
    ...
    for e in encs:
        ...
        for i in range(e.assistants):
            ...
            staff.append(
                f"{e.log_id}|S{9000 + i * 37 + hash(e.log_id) % 90:05d}|"
                f"Assistant Surgeon|{spec}|{i + 2}"
            )
...
def main() -> None:
    ...
    # downstream can tell.
    for owned in ("clinical", "remittance"):
